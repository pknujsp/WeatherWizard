...
@Composable
fun SimpleDailyForecastScreen(weatherInfoViewModel: WeatherInfoViewModel, navigate: (NestedWeatherRoutes) -> Unit) {
    val dailyForecast by weatherInfoViewModel.simpleDailyForecast.collectAsStateWithLifecycle()

    dailyForecast.onSuccess {
        SimpleWeatherScreenBackground(
            CardInfo(
                title = stringResource(id = io.github.pknujsp.weatherwizard.core.common.R.string.daily_forecast),
                buttons = listOf(
