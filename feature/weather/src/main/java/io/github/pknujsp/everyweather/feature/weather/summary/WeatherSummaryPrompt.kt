...
class WeatherSummaryPrompt(
    ...
    ## 위에 제공된 날씨 데이터를 종합적으로 분석(마크다운 형식으로 날씨 데이터가 구성되어 있음)        
            
    역할: 방대한 날씨 데이터를 심도깊게 분석하여, 사용자에게 필요한 정보를 간결하고 명확하게 전달하는 전문가
    ...
    - 실제 기상 캐스터, 날씨 전문가가 응답 품질을 평가하므로 잘 작성해야 합니다.
    - 실제 전문가가 평가하였을 때 평가 점수가 100점 만점에 100점이 되도록 해야 합니다.
    
    응답 지시사항: 
    - 단락 별로 응답은 최소 50자 이상으로 작성합니다.
    - 읽기 쉽도록 행 개행을 사용합니다.
