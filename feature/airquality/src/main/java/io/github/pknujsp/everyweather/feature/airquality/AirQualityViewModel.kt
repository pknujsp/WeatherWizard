    ...
    constructor(
        ...
        private fun createModel(airQuality: AirQualityEntity): SimpleAirQuality =
            airQuality.let {
                ...
                val info =
                    SimpleAirQuality.Info(
                        dataMeasurementTime = dataMeasurementTime.format(DateTimeFormatter.ofPattern("M/d E HH:mm")),
                        stationName = it.info.stationName,
                    )
                val current =
                    SimpleAirQuality.Current(
                        it.current.aqi,
                        it.current.co,
                        it.current.no2,
                        it.current.o3,
        ...
        private fun createDailyForecast(airQualityEntity: AirQualityEntity) =
            airQualityEntity.dailyForecast.items.run {
                val indexes =
                    map { item ->
                ...
                indexes.map {
                    SimpleAirQuality.DailyItem(
                        dateTime = LocalDate.parse(it.first.date),
                        aqi = it.second,
