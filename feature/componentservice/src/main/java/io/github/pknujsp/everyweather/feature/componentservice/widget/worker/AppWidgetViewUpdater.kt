...
class AppWidgetViewUpdater(
    ...
    suspend operator fun invoke(
        ...
        val widgets =
            widgetRepository.get(widgetIds ?: widgetManager.installedAllWidgetIds).filterNot {
    ...
    private fun updateViews(
        ...
        widgets.filter { it.locationType is LocationType.CurrentLocation }.let { filtered ->
            if (!filtered.checkPrimaryRequiredFeatures(
                    arrayOf(
        ...
        val remoteViewsCreatorMap: Map<WidgetType, WidgetRemoteViewsCreator<RemoteViewUiModel>> =
            widgets.filterNot {
        ...
        for (widget in widgets) {
            ...
            val remoteView =
                if (widget is SavedWidgetContentState.Success) {
