...
internal class AirQualityRepositoryImpl(
    ...
    override suspend fun getAirQuality(
        ...
        val result =
            load(latitude, longitude).map { response ->
                response.data.run {
                    val current =
                        AirQualityEntity.Current(
                            aqi =
                                AirQualityValueType(
                                    value = aqi.toInt(),
                    ...
                    val items: List<AirQualityEntity.DailyForecast.Item> =
                        forecast.daily.run {
                            val o3 = o3.map()
                            val pm10 = pm10.map()
                            val pm25 = pm25.map()
    ...
    private fun List<AqiCnResponse.Data.Forecast.Daily.ForecastPollutant>.map() =
        associate {
            LocalDate.parse(it.day) to
                AirQualityEntity.DailyForecast.Item.Pollutant(
                    avg =
                        AirQualityValueType(
                            value = it.avg.toInt(),
    ...
    private suspend fun getCache(key: Int): AirQualityEntity? =
        when (val cacheState = cacheManager.get(key)) {
