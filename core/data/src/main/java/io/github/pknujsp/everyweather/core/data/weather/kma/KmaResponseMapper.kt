...
internal class KmaResponseMapper : WeatherResponseMapper<WeatherEntityModel> {
                ...
                HourlyForecastEntity.Item(
                    ...
                    precipitationVolume =
                        PrecipitationValueType(
                            if (item.rainVolume.isNaN() and item.snowVolume.isNaN()) PrecipitationValueType.none.value else item.rainVolume.toLeastZero() + item.snowVolume.toLeastZero(),
    ...
    private fun mapDailyForecast(response: KmaDailyForecastResponse): DailyForecastEntity {
        val dayItems: List<DailyForecastEntity.DayItem> =
            response.items.map { dayItem ->
                DailyForecastEntity.DayItem(
                    ...
                    items =
                        listOfNotNull(dayItem.amValues, dayItem.pmValues, dayItem.singleValues).map { item ->
