...
class GetHourlyForecastToCompareUseCase
    @Inject
    constructor(
        private val weatherDataRepository: WeatherDataRepository,
    ) : BaseGetForecastToCompareUseCase<ToCompareHourlyForecastEntity> {
        override suspend fun invoke(requests: List<WeatherDataRequest.Request>): Result<ToCompareHourlyForecastEntity> {
            return requests.map { request ->
                ...
                if (responses.all { it.second.isSuccess }) {
                    val entities =
                        responses.map {
                            it.first to
                                (it.second.getOrThrow().list.first() as HourlyForecastEntity).items.mapByWeatherProvider(it.first)
                                    .map { item ->
                                        ToCompareHourlyForecastEntity.Item(
        ...
        private fun List<HourlyForecastEntity.Item>.mapByWeatherProvider(weatherProvider: WeatherProvider) =
            if (weatherProvider == WeatherProvider.MetNorway) {
                ...
                for (i in (size / 2)..<size) {
                    val diff =
                        map.getOrPut(this[i].dateTime.value) {
