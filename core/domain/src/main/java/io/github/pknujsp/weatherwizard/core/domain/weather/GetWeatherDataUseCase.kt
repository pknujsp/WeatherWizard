...
class GetWeatherDataUseCase @Inject constructor(
    ...
    suspend operator fun invoke(request: WeatherDataRequest.Request): ResponseState {
        return request.run {
            ...
            if (responseList.all { it.second.isSuccess }) {
                ResponseState.Success(requestId,
                    coordinate,
                    weatherDataProvider,
                    ResponseEntity(weatherDataMajorCategories, responseList.map { it.second.getOrThrow() }))
                ...
                ResponseState.PartiallySuccess(requestId,
                    coordinate,
                    weatherDataProvider,
                    ResponseEntity(weatherDataMajorCategories - WeatherDataMajorCategory.AIR_QUALITY,
