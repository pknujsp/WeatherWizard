...
class NewGraph(
    ...
    fun createNewGraph(height: Float): List<List<LinePoint>> {
        ...
        val points = values.map { vals ->
            val centerPoints = vals.mapIndexed { i, value ->
    ...
    private fun calculateYPosition(value: Int, minValue: Int, valueLength: Int) = 1f - ((value - minValue).toFloat() / valueLength)
