package io.github.pknujsp.everyweather.core.widgetnotification.widget.worker.model

import io.github.pknujsp.everyweather.core.data.mapper.JsonParser
import io.github.pknujsp.everyweather.core.data.widget.WidgetResponseDBModel
...
class WidgetRemoteViewUiState(
    ...
    private companion object {
        fun realTypeParseToByteArray(
            jsonParser: JsonParser,
            majorWeatherEntityType: MajorWeatherEntityType,
            model: WeatherEntityModel,
            ...
            MajorWeatherEntityType.YESTERDAY_WEATHER ->
                jsonParser.parseToByteArray<YesterdayWeatherEntity>(
                    model as YesterdayWeatherEntity,
                )
    ...
    fun toWidgetResponseDBModel(jsonParser: JsonParser) =
        WidgetResponseDBModel(
            entities =
                model!!.map { entity ->
                    WidgetResponseDBModel.EntityWithWeatherProvider(
                        weatherProvider = entity.weatherProvider.key,
                        entities =
                            entity.entity.export(widget.widgetType.categories.toSet()).map {
