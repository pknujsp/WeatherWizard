...
class WidgetLocalDataSourceImpl(
    ...
    private val compressionTool: CompressionTool,
) : WidgetLocalDataSource {
    override suspend fun add(widgetDto: WidgetDto): Int = widgetDao.insert(widgetDto).toInt()

    override suspend fun getAll(excludesResponseData: Boolean): List<WidgetDto> =
        widgetDao.run {
    ...
    override suspend fun updateResponseData(
        ...
        val finalStatus =
            if (compressed == null && status == WidgetStatus.RESPONSE_SUCCESS) {
